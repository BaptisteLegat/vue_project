<template>
    <div class="content-container">
        ...
        <v-row class="card-grid justify-center"> <!-- Ajout de la classe justify-center -->
            ...
            if (this.selectedColor === "" && this.selectedSize === "" && this.searchQuery === "") {
                ...
                return this.products.filter((product) => {
                    ...
                    const colorMatch =
                        ...
                        selectedColors.some((colorId) => product.color["@id"] === `/api/colors/${colorId}`);
                    const sizeMatch =
                        selectedSizes.length === 0 || selectedSizes.some((sizeId) => product.size["@id"] === `/api/sizes/${sizeId}`);
    ...
    methods: {
        ...
        updateSelectedFilter(selections) {
            ...
            if (this.selectedSize.length > 0) {
                apiUrl +=
                    "size[]=" + this.selectedSize.map((sizeId) => `/api/sizes/${sizeId}`).join("&size[]=") + "&";
            }
        ...
        handlePaginationClick(pageNumber) {
            ...
            if (this.selectedColor.length > 0) {
                apiUrl +=
                    "&color[]=" +
                    this.selectedColor.map((colorId) => `/api/colors/${colorId}`).join("&color[]=") + "&";
            }
            if (this.searchQuery !== "") {
                apiUrl += "&name=" + this.searchQuery + "&";
            }
        ...
        buildApiUrl() {
            ...
            if (this.selectedColor.length > 0) {
                apiUrl +=
                    "color[]=" + this.selectedColor.map((colorId) => `/api/colors/${colorId}`).join("&color[]=");
            }
            if (this.searchQuery !== "") {
            ...
            if (this.selectedSize.length > 0) {
                apiUrl +=
                    ...
                    "size[]=" +
                    this.selectedSize.map((sizeId) => `/api/sizes/${sizeId}`).join("&size[]=");
            }
